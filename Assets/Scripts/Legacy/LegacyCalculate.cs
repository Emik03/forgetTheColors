...
namespace LegacyForgetTheColors
{
    ...
    /// </summary>
    sealed class LegacyCalculate
    {
        public void FinalStage(ref int moduleId, ref int maxStage, ref List<int> calculatedValues, ref float sum, ref TextMesh[] NixieTexts, ref bool canSolve, ref sbyte solution)
        {
            ...
            Debug.LogFormat("[Forget The Colors #{0}]: First five digits of cos-1({1}) is {2}.", moduleId, LegacyEase.Modulo(Mathf.Abs(sum) / 100000, 1), Math.Truncate(Mathf.Acos((float)LegacyEase.Modulo(Mathf.Abs(sum) / 100000, 1)) * Mathf.Rad2Deg));
            solution = (sbyte)(Mathf.Acos((float)LegacyEase.Modulo(Mathf.Abs(sum) / 100000, 1)) * Mathf.Rad2Deg);
        ...
        public void NextStage(ref int moduleId, ref int currentStage, ref TextMesh[] DisplayTexts, ref TextMesh[] NixieTexts, ref TextMesh GearText, ref bool colorblind, ref LegacyRule[][] rules, ref int[] colorValues, ref KMBombInfo BombInfo, ref int maxStage, ref List<string> ruleColor, ref List<int> calculatedValues, ref List<int> sineNumber)
        {
            ...
            Debug.LogFormat("[Forget The Colors #{0}]: Large Display: {1}. Cylinders (left-to-right): {2}, {3}, and {4}. Nixies: {5} and {6}. Gear: {7} and {8}.", moduleId, DisplayTexts[0].text, LegacyStrings.ColorLog[colorValues[0]], LegacyStrings.ColorLog[colorValues[1]], LegacyStrings.ColorLog[colorValues[2]], NixieTexts[0].text, NixieTexts[1].text, GearText.text[Convert.ToByte(colorblind)], LegacyStrings.ColorLog[colorValues[3]]);
            ...
            if (rules == null)
            {
                ...
                for (byte i = 0; i < colorValues.Length - 1; i++)
                {
                    ...
                    Debug.LogFormat("[Forget The Colors #{0}]: Applying the {1}-colored cylinder on the first table, the nixies are now {2} and {3}.", moduleId, LegacyStrings.ColorLog[colorValues[i]], nixie1, nixie2);
                }
            }
            ...
            else
            {
                for (byte i = 0; i < colorValues.Length - 1; i++)
                {
                    LegacyRule rule = rules[0][colorValues[i]];
                    ...
                    switch (rule.Operator)
                    {
                        ...
                        case 4: if (rule.Cylinder != 0) nixie1 = (sbyte)LegacyEase.Modulo(nixie1, rule.Cylinder); break;
                    }
                    ...
                    switch (rule.Operator)
                    {
                        ...
                        case 4: if (rule.Cylinder != 0) nixie2 = (sbyte)LegacyEase.Modulo(nixie2, rule.Cylinder); break;
                    }
            ...
            //modulo
            nixie1 = (sbyte)LegacyEase.Modulo(nixie1, 10);
            nixie2 = (sbyte)LegacyEase.Modulo(nixie2, 10);
            ...
            int lsd = (byte)LegacyEase.Modulo(nixie1 + nixie2 + int.Parse(GearText.text[Convert.ToByte(colorblind)].ToString()), 10);
            ...
            Debug.LogFormat("[Forget The Colors #{0}]: Start on gear color ({1}), move up left nixie ({2}) which lands on {3}, then move down right nixie ({4}) which lands us on {5}.", moduleId, LegacyStrings.ColorLog[colorValues[3]], nixie1, LegacyStrings.ColorLog[(int)LegacyEase.Modulo(colorValues[3] - nixie1, 10)], nixie2, LegacyStrings.ColorLog[(int)LegacyEase.Modulo(colorValues[3] - nixie1 + nixie2, 10)]);

            if (rules == null)
            {
                //this will run through the changes applied to the gear during step 2 of second page on manual
                switch ((int)LegacyEase.Modulo(colorValues[3] - nixie1 + nixie2, 10))
                {
            ...
            else
            {
                ...
                foreach (string module in BombInfo.GetModuleNames())
                    if (LegacyStrings.Ignore.Contains(module))
                        ignoredCount++;

                LegacyRule rule = rules[1][(int)LegacyEase.Modulo(colorValues[3] - nixie1 + nixie2, 10)];

                //the smaller display is used as a debug displayer in the editor, which cannot be parsed
                int[] edgework = new int[22] { BombInfo.GetBatteryCount(), BombInfo.GetBatteryCount(Battery.AA) + BombInfo.GetBatteryCount(Battery.AAx3) + BombInfo.GetBatteryCount(Battery.AAx4), BombInfo.GetBatteryCount(Battery.D), BombInfo.GetBatteryHolderCount(), BombInfo.GetIndicators().Count(), BombInfo.GetOnIndicators().Count(), BombInfo.GetOffIndicators().Count(), BombInfo.GetPortPlateCount(), BombInfo.GetPorts().Distinct().Count(), BombInfo.GetPorts().Count() - BombInfo.GetPorts().Distinct().Count(), BombInfo.GetPortCount(), BombInfo.GetSerialNumberNumbers().First(), BombInfo.GetSerialNumberNumbers().Last(), BombInfo.GetSerialNumberNumbers().Count(), BombInfo.GetSerialNumberLetters().Count(), BombInfo.GetSolvedModuleNames().Count(), maxStage, BombInfo.GetModuleNames().Count(), BombInfo.GetSolvableModuleNames().Count() - BombInfo.GetSolvedModuleNames().Count(), maxStage - BombInfo.GetSolvedModuleNames().Where(m => !LegacyStrings.Ignore.Contains(m)).Count(), Application.isEditor ? 0 : int.Parse(DisplayTexts[1].text), int.Parse(DisplayTexts[0].text) };
                ...
                switch (rule.Operator)
                {
                    ...
                    case 4: if (edgework[rule.Edgework] != 0) lsd = (int)LegacyEase.Modulo(nixie2, edgework[rule.Edgework]); break;
                }
            }

            ruleColor[currentStage] = LegacyStrings.ColorLog[(int)LegacyEase.Modulo(colorValues[3] - nixie1 + nixie2, 10)];
            Debug.LogFormat("[Forget The Colors #{0}]: Apply the color rule {1} to the sum of the first nixie ({2}) + the second nixie ({3}) + the gear number ({4}). This gives us {5}. Modulo 10, its value is {6}.", moduleId, LegacyStrings.ColorLog[(int)LegacyEase.Modulo(colorValues[3] - nixie1 + nixie2, 10)], nixie1, nixie2, GearText.text, nixie1 + nixie2 + int.Parse(GearText.text.Last().ToString()), LegacyEase.Modulo(lsd, 10));

            //modulo
            lsd = (int)LegacyEase.Modulo(lsd, 10);
            ...
            if (LegacyEase.Modulo(Math.Abs(sin), 1000) == 999)
                if (sin > 0)
                    sin = (sin + 1) % 100000;
                else
                    sin = (sin - 1) % 100000;
            ...
            if (LegacyEase.Modulo(cos, 1000) == 999)
                cos = (int)LegacyEase.Modulo(cos + 1, 100000);
