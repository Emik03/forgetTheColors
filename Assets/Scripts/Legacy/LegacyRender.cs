...
namespace LegacyForgetTheColors
{
    ...
    /// </summary>
    sealed class LegacyRender
    {
        public bool Animate(ref Transform Gear, int angleIncreasePerSolve, ref int currentAngle, ref bool allowCycleStage, ref float ease, ref bool isRotatingGear, ref bool canSolve, ref bool solved, ref KMSelectable[] Selectables, ref Transform Key, ref bool isRotatingKey, ref int stage, ref KMBombInfo BombInfo)
        {
            ...
            if (solved)
            {
                ...
                if (ease <= 1)
                {
                    ...
                    Selectables[2].transform.localRotation = Quaternion.Euler(0, LegacyEase.BackOut(ease) * 420, 0);
                    Key.localScale = new Vector3(LegacyEase.ElasticOut(ease) * 0.5f, 1, LegacyEase.ElasticOut(ease) * 0.5f);
                }
                ...
                else if (ease <= 2)
                {
                    ...
                    Selectables[2].transform.localPosition = new Vector3(0, (LegacyEase.BackIn(ease - 1) * -3) - 0.91f, 0);
                    Key.localScale = new Vector3((1 - LegacyEase.ElasticIn(ease - 1)) / 2, 1, (1 - LegacyEase.ElasticIn(ease - 1)) / 2);
                }
            ...
            {
                ease += 0.04f;
                Selectables[2].transform.localRotation = Quaternion.Euler(0, (LegacyEase.ElasticOut(ease) - ease) * 69, 0);
            ...
            else if (Gear.localRotation.y != angleIncreasePerSolve + currentAngle && !allowCycleStage)
            {
                ease += 0.025f;
                ...
                if (canSolve)
                {
                    Gear.localRotation = Quaternion.Euler(0, currentAngle % 360 * Math.Abs(LegacyEase.CubicOut(ease) - 1), 0);
                ...
                {
                    Gear.localRotation = Quaternion.Euler(0, LegacyEase.CubicOut(ease) * angleIncreasePerSolve + currentAngle, 0);
        ...
        public void Update(ref bool canSolve, ref TextMesh[] DisplayTexts, ref TextMesh GearText, ref Renderer[] ColoredObjects, ref Texture[] ColorTextures, ref Transform[] CylinderDisks, ref int[] colorValues, ref bool colorblind, ref int maxStage, ref int stage)
        {
            ...
            else
            {
                ...
                //set gear size
                GearText.characterSize = 0.05f - (Convert.ToByte(colorblind) * 0.025f);
                ...
                if (colorblind)
                {
                    ...
                    if (colorValues[3] != 7)
                        GearText.text = LegacyStrings.ColorLog[colorValues[3]].First() + GearText.text.Last().ToString();
                    else
        ...
        public void UpdateCycleStage(ref TextMesh[] DisplayTexts, ref List<short> largeDisplay, ref int stage, ref TextMesh[] NixieTexts, ref List<byte> nixies, ref TextMesh GearText, ref List<byte> gear, ref bool colorblind, ref int maxStage, ref List<string> gearColor, ref Renderer[] ColoredObjects, ref Texture[] ColorTextures, ref List<byte> cylinder, ref Transform[] CylinderDisks)
        {
            ...
            DisplayTexts[1].text = LegacyEase.Modulo(stage, 100).ToString();
            ...
            GearText.characterSize = 0.05f - (Convert.ToByte(colorblind) * Convert.ToByte(maxStage != stage) * 0.025f);
