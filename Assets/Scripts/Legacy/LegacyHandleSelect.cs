    ...
    sealed class LegacyHandleSelect
    {
        ...
        public void Press(byte btn, ref KMAudio Audio, ref KMSelectable[] Selectables, ref bool solved, ref bool canSolve, ref bool allowCycleStage, ref int stage, ref int maxStage, ref TextMesh[] DisplayTexts, ref List<short> largeDisplay, ref TextMesh[] NixieTexts, ref List<byte> nixies, ref TextMesh GearText, ref List<byte> gear, ref bool colorblind, ref List<string> gearColor, ref Renderer[] ColoredObjects, ref Texture[] ColorTextures, ref List<byte> cylinder, ref Transform[] CylinderDisks, ref int[] colorValues, ref sbyte debugPointer, ref int moduleId, ref LegacyRule[][] rules, ref KMBombInfo BombInfo, ref List<string> ruleColor, ref List<int> calculatedValues, ref List<int> sineNumber, ref float sum, ref sbyte solution, ref bool isRotatingGear, ref int currentAngle, int angleIncreasePerSolve, ref float ease, ref KMBombModule Module, ref bool isRotatingKey)
        {
            ...
            if (allowCycleStage)
            {
                switch (btn)
                {
                    case 0: stage = (int)LegacyEase.Modulo(stage - 1, maxStage); moduleRender.UpdateCycleStage(ref DisplayTexts, ref largeDisplay, ref stage, ref NixieTexts, ref nixies, ref GearText, ref gear, ref colorblind, ref maxStage, ref gearColor, ref ColoredObjects, ref ColorTextures, ref cylinder, ref CylinderDisks); break;
                    case 1: stage = (int)LegacyEase.Modulo(stage + 1, maxStage); moduleRender.UpdateCycleStage(ref DisplayTexts, ref largeDisplay, ref stage, ref NixieTexts, ref nixies, ref GearText, ref gear, ref colorblind, ref maxStage, ref gearColor, ref ColoredObjects, ref ColorTextures, ref cylinder, ref CylinderDisks); break;
            ...
            else if (btn != 2)
            {
                ...
                if (canSolve || !Application.isEditor)
                    NixieTexts[btn].text = LegacyEase.Modulo(int.Parse(NixieTexts[btn].text) + 1, 10).ToString();
                ...
                else
                {
                    ...
                    if (btn == 1)
                        switch (debugPointer)
                        {
                            case 0: DisplayTexts[0].text = LegacyEase.Modulo(int.Parse(DisplayTexts[0].text) + 100, 1000).ToString(); break;
                            case 1: DisplayTexts[0].text = LegacyEase.Modulo(int.Parse(DisplayTexts[0].text) + 10, 1000).ToString(); break;
                            case 2: DisplayTexts[0].text = LegacyEase.Modulo(int.Parse(DisplayTexts[0].text) + 1, 1000).ToString(); break;
                            case 3: colorValues[0] = (int)LegacyEase.Modulo(colorValues[0] + 1, 10); break;
                            case 4: colorValues[1] = (int)LegacyEase.Modulo(colorValues[1] + 1, 10); break;
                            case 5: colorValues[2] = (int)LegacyEase.Modulo(colorValues[2] + 1, 10); break;
                            case 6: GearText.text = LegacyEase.Modulo(int.Parse(GearText.text[Convert.ToByte(colorblind)].ToString()) + 1, 10).ToString(); break;
                            case 7: colorValues[3] = (int)LegacyEase.Modulo(colorValues[3] + 1, 10); break;
                            case 8: NixieTexts[0].text = LegacyEase.Modulo(int.Parse(NixieTexts[0].text) + 1, 10).ToString(); break;
                            case 9: NixieTexts[1].text = LegacyEase.Modulo(int.Parse(NixieTexts[1].text) + 1, 10).ToString(); break;
                            case 10: stage++; break;

                            case 11:
                                //souvenir
                                gear[stage] = byte.Parse(GearText.text.Last().ToString());
                                gearColor[stage] = LegacyStrings.ColorLog[colorValues[3]];
                                ...
                                calculate.FinalStage(ref moduleId, ref maxStage, ref calculatedValues, ref sum, ref NixieTexts, ref canSolve, ref solution);
                                break;
                        }

                    //left nixie changes which value is selected
                    else
                        debugPointer = (sbyte)LegacyEase.Modulo(++debugPointer, 12);

                    DisplayTexts[1].text = LegacyStrings.DebugText[debugPointer];
            ...
            {
                //debugging
                if (Application.isEditor && !canSolve)
                {
                    //souvenir
                ...
                else if (int.Parse(string.Concat(NixieTexts[0].text, NixieTexts[1].text)) == solution)
                {
                    ...
                    Debug.LogFormat("[Forget The Colors #{0}]: {1}; module solved!", moduleId, LegacyStrings.WinPhrases[Rnd.Range(0, LegacyStrings.WinPhrases.Length)]);
                    Module.HandlePass();
                }
                ...
                else
                {
                    ...
                    if (int.Parse(string.Concat(NixieTexts[0].text, NixieTexts[1].text)) <= 90)
                        Debug.LogFormat("[Forget The Colors #{0}]: {1}; you submitted {2} when I expected {3}.", moduleId, LegacyStrings.FailPhrases[Rnd.Range(0, LegacyStrings.FailPhrases.Length)], NixieTexts[0].text + NixieTexts[1].text, solution);

                    //if submitting an impossible answer
                    else
                        Debug.LogFormat("[Forget The Colors #{0}]: Wh- why would you submit {1}? Why would you submit {1}?! Do you even know how I work?! That answer isn't even possible... You submitted {1} when I expected {2}.", NixieTexts[0].text + NixieTexts[1].text, solution);
