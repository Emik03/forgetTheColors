...
public class FTC : MonoBehaviour
{
    ...
    private HandleSelect handleSelect = new HandleSelect();
    private Init init = new Init();
    private ModuleRender moduleRender = new ModuleRender();
    ...
    {
        if (moduleRender.Animate(ref Gear, _angleIncreasePerSolve, ref _currentAngle, ref _allowCycleStage, ref _ease, ref _isRotatingGear, ref _canSolve, ref solved, ref Selectables, ref Key, ref _isRotatingKey, ref stage, ref BombInfo) &&
            !solved && !_allowCycleStage && !_isRotatingGear && stage < BombInfo.GetSolvedModuleNames().Where(module => !Strings.Ignore.Contains(module)).Count())
        {
            //generate a stage
